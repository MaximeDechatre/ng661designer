...
import org.polarsys.ng661designer.part2.DocumentRoot;
...
public class NG661DesignerModelUtils {
	...
	public void setScript(EObject element, String script) {
		...
		} else if (element instanceof ScxmlScxmlType || element instanceof ScxmlTransitionType
				|| element instanceof ScxmlOnexecuteType) {
	 ...
	 */
	public List<ScxmlStateType> getTransitionTarget(ScxmlTransitionType transition, ScxmlScxmlType scxmlType) {
		List<ScxmlStateType> result = Lists.newArrayList();
		String target = transition.getTarget();
		if (target != null) {
			for (Iterator<EObject> iterator = scxmlType.eAllContents(); iterator.hasNext();) {
	 ...
	 */
	public List<ComponentDefinitionType> getAllComponentDefinitions(TreeType element) {
		...
		if (session != null) {
			for (Resource resource : session.getSemanticResources()) {
				if (!resource.getContents().isEmpty() && resource.getContents().get(0) instanceof DocumentRoot
						&& ((DocumentRoot) resource.getContents().get(0)).getComponentDefinition() != null
						&& !Strings.isNullOrEmpty(
								((DocumentRoot) resource.getContents().get(0)).getComponentDefinition().getName())) {
	 ...
	 */
	public WidgetComposition addWidgetComposition(TreeType tree, ComponentDefinitionType componentDefinition) {
		if (Strings.isNullOrEmpty(componentDefinition.getName())) {
			MessageDialog.openError(Display.getDefault().getActiveShell(), "Add widget composition",
					"The reused representation has no name, it will not be imported.");
			return null;
		}
		...
		tree.getGroup().add(widgetComposition);
		// add import
		List<ImportType> imports = tree.getImport();
		if (!contains(imports, componentDefinition)) {
			...
			EObject eContainer = tree.eContainer();
			if (eContainer instanceof ComponentDefinitionType) {
				DefinitionType definition = ((ComponentDefinitionType) eContainer).getDefinition();
				if (definition != null) {
					definition = Part2Factory.eINSTANCE.createDefinitionType();
					((ComponentDefinitionType) eContainer).setDefinition(definition);
				}
				definition.getImport().add(importType);
			}
		}
	...
	private boolean contains(List<ImportType> imports, ComponentDefinitionType componentDefinition) {
		for (ImportType importType : imports) {
			if (importType.getFile() != null
					&& importType.getFile().equals(componentDefinition.eResource().getURI().lastSegment())) {
				return true;
			}
		}
		return false;
	...
	public PropertyValue addPropertyValue(WidgetComposition widgetComposition, PropertyType property) {
		if (Strings.isNullOrEmpty(property.getName())) {
			MessageDialog.openError(Display.getDefault().getActiveShell(), "Add property value",
					"The reused property has no name, it will not be imported.");
			return null;
		}
	 ...
	 * @param widgetComposition
	 *            WidgetComposition
	 * @return all PropertyType in widgetComposition.
	 */
	public List<PropertyType> getProperties(WidgetComposition widgetComposition) {
		...
		if (widgetComposition.getRepresentation() != null
				&& widgetComposition.getRepresentation().getInterface() != null) {
